...
class GaussianSmearing(nn.Module):
    def __init__(self, start=0.0, stop=10.0, num_gaussians=50):
        super().__init__()
        self.stop = stop
        offset = torch.linspace(start, stop, num_gaussians)  # discrete enum
...
def embed_compose(compose_feature, compose_pos, idx_ligand, idx_protein,
                                      ...
                                      emb_dim):
    # print(compose_feature[idx_protein].shape)
    h_ligand = ligand_atom_emb(compose_feature[idx_ligand], compose_pos[idx_ligand])  #(length, atom_classes 27, atom_xyz 3)
    h_protein = protein_atom_emb(compose_feature[idx_protein], compose_pos[idx_protein])
    # print(compose_feature[idx_ligand].shape)
    # print(compose_pos[idx_ligand].shape)
    # print(h_protein[1].shape)
    ...
    h_sca[idx_ligand], h_sca[idx_protein] = h_ligand[0], h_protein[0] # (len,256)
    h_vec[idx_ligand], h_vec[idx_protein] = h_ligand[1], h_protein[1] # (len,64,3)
    # print(h_sca.shape)
    # print(h_vec.shape)
    return [h_sca, h_vec]
