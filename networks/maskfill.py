...
from .MFC_classifier import get_field_vn
from .common import *
...
from data_pre.record import unique
...
class MaskFillModelVN(Module):
    ...
    def __init__(self, config, num_classes, num_bond_types, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        self.focal_loss = focal_loss(alpha=[1, 9], gamma=2, num_classes=2)
    ...
    def sample_element_and_bond(self, 
        ...
        y_query_pred, y_cls, edge_pred, y_outelec, pre_elec_radi = self.query_position(
        ...
        if n_samples < 0:
            ...
            # y_query_pred = y_cls# + y_query_pred# + Radi_att * y_query_pred #+ outelec_att * y_query_pred
            ...
            # y_query_pred = y_cls# + y_query_pred#Elec_att * y_query_pred + y_query_pred# + outelec_att * y_query_pred
            y_query_preds = F.softmax(y_cls, dim=-1)
            element_pred = y_query_preds.multinomial(n_samples, replacement=True).reshape(-1)  # n_query * n_samples
            idx_parent = torch.repeat_interleave(torch.arange(n_query), n_samples, dim=0).to(compose_pos.device)
            element_prob = y_query_preds[idx_parent, element_pred]
        # # 5: determine bonds
        if n_samples < 0:
    ...
    def sample_init_element(self, 
        ...
        y_query_pred, y_cls, _, y_outelec, pre_elec_radi = self.field(
            pos_query = pos_generated,
            edge_index_query = [],
        ...
        if n_samples < 0:
            ...
            has_atom_prob = 1 - 1 / (1 + torch.exp(y_query_pred).sum(-1))
            # y_query_pred = y_cls# + y_query_pred# + Radi_att * y_query_pred# + outelec_att * y_query_pred
            y_query_pred = F.softmax(y_cls, dim=-1)
    ...
    def get_loss(self, pos_real, y_real, y_outelec_num, y_elec_radi, pos_fake,  # query real positions,
        ...
        # print(y_elec_radi)
        h_compose = embed_compose(compose_feature, compose_pos, idx_ligand, idx_protein,
                                      self.ligand_atom_emb, self.protein_atom_emb, self.emb_dim) # all feature, contain
        # print(h_compose[0].shape)
        # print(h_compose[1].shape)
        # print(compose_feature.shape)
        # print(compose_pos.shape)
        # # Encode compose
        h_compose = self.encoder(
            node_attr = h_compose,
            pos = compose_pos,
        ...
        # print(h_compose[0].shape) #torch.Size([2202, 256])
        # print(compose_pos.shape) #torch.Size([2202, 3])
        # # 3: Element and bonds of the new position atoms
        y_real_pred, y_cls, edge_pred, y_outelec, pre_elec_radi = self.field(
            pos_query = pos_real,
        ...
        # # fake positions
        y_fake_pred, _, _, y_fake_outelec, pre_fake_elec_radi = self.field(
            pos_query = pos_fake,
            edge_index_query = [],
            pos_compose = compose_pos,
            node_attr_compose = h_compose,
        ...
        loss_cls = self.smooth_cross_entropy(y_cls, y_real.argmax(-1)).clamp_max(10.)   # Classes
        # print(loss_cls)
        loss_elec = F.cross_entropy(y_outelec, y_outelec_num.argmax(-1)).clamp_max(10.)  # y_outelec_num
        # print(loss_elec)
        loss_elec_radi = self.focal_loss(pre_elec_radi, y_elec_radi.argmax(-1)).clamp_max(10.)
        # print(loss_elec_radi)
        # print(self.smooth_cross_entropy(pre_elec_radi, y_elec_radi.argmax(-1)).clamp_max(10.))
        # print("\n")
        # print(edge_label)
        loss_edge = F.cross_entropy(edge_pred, edge_label).clamp_max(10.)
        # real and fake loss

        # print(pre_elec_radi_p)#torch.Size([108, 2])
        ...
        loss = (torch.nan_to_num(loss_frontier)
                    ...
                    + torch.nan_to_num(loss_elec)
                    + torch.nan_to_num(loss_elec_radi)
        )

        return loss, loss_frontier, loss_pos, loss_cls, loss_edge, loss_real, loss_fake, torch.nan_to_num(loss_surf), torch.nan_to_num(loss_elec), torch.nan_to_num(loss_elec_radi)  # loss_notpos
    ...
    def query_position(self, pos_query, h_compose, compose_pos,
        ...
        return y_real_pred, y_cls, edge_pred, y_outelec, pre_elec_radi
...
# [Validate]  Iter 5000 | Loss 5.530429 | Loss(Fron) 0.137547 | Loss(Pos) 2.946034 | Loss(Cls) 0.869344 | Loss(Edge) 0.063739 | Loss(Real) 0.116723 | Loss(Fake) 0.223065  | Loss(Surf) 0.076199  | Loss(Elec) 0.746903  | Loss(Radi) 0.350874
#