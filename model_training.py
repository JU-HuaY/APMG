...
from networks.maskfill import MaskFillModelVN
from data_pre.data_preprocess import *
from data_pre.ATPG import *
from data_pre.record import *
from data_pre.train_process import *
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default='cuda', help='device id, format is like 0,1,2')
    parser.add_argument('--logdir', type=str, default='./logs_elec_cls_bond')
    ...
    def train(it):
        ...
        optimizer.zero_grad()
        batch = next(train_iterator).cuda(device=device_ids[0])#.to(args.device)
        # print(batch.idx_ligand_ctx_in_compose)
        # print(batch.compose_feature.float().shape)
        compose_noise = torch.randn_like(batch.compose_pos) * config.train.pos_noise_std

        loss, loss_frontier, loss_pos, loss_cls, loss_edge, loss_real, loss_fake, loss_surf, loss_elec, loss_elec_radi = model.get_loss(

            pos_real = batch.pos_real,
            y_real = batch.cls_real.long(),
            y_outelec_num = batch.outelec_num,
            y_elec_radi = batch.elec_radi,
        ...
        if config.train.use_apex:
            ...
            loss.backward()
        orig_grad_norm = clip_grad_norm_(model.parameters(), config.train.max_grad_norm)  # 5% running time
        optimizer.step()

    def validate(it):
        sum_loss, sum_n = np.zeros(5 + 2 + 1 + 1 + 1), 0   # num of loss
        with torch.no_grad():
            model.eval()
            for batch in tqdm(val_loader, desc='Validate'):
                batch = batch.cuda(device=device_ids[0])#.to(args.device)
        ...
        logger.info('[Validate]  Iter %d | Loss %.6f | Loss(Fron) %.6f | Loss(Pos) %.6f | Loss(Cls) %.6f | Loss(Edge) %.6f | Loss(Real) %.6f | Loss(Fake) %.6f  | Loss(Surf) %.6f  | Loss(Elec) %.6f  | Loss(Radi) %.6f' % (
            it, *avg_loss,
        ))
        ...
        writer.add_scalar('val/loss_elec', avg_loss[8], it)
        writer.add_scalar('val/loss_elec_radi', avg_loss[9], it)
        writer.flush()
        return avg_loss
